...
export default function CarteirinhaBack({
  ...
  const credential = getOrCreateUserCredential(user.id, user, student);
  ...
  return (
    <div
      style={bgStyle}
      className="relative w-full aspect-[1.58/1] rounded-2xl border border-neutral-800 shadow-2xl p-5 flex flex-col justify-between overflow-hidden group transition-all duration-300 text-left"
    >
        ...
        {config.versoLogoExibir !== false && (
          <div className="flex items-center gap-2.5">
            <div
              ...
              style={{ width: `${config.versoLogoTamanho || 36}px`, height: `${config.versoLogoTamanho || 36}px` }}
            >
              {config.logoPrincipalUrl ? (
                <img
                  src={config.logoPrincipalUrl}
            ...
            <div>
              ...
              <span className="text-[7.5px] text-neutral-400 font-bold uppercase tracking-wider block">
      ...
      {/* Main Body - QR Code & Auth Code */}
      <div className="my-auto z-10 flex items-center justify-between gap-4 py-1">
        {/* Individual QR Code */}
        <div className="flex flex-col items-center bg-white/95 p-1.5 rounded-xl border border-neutral-700 shadow-xl shrink-0">
          {qrDataUrl ? (
            <img
              src={qrDataUrl}
              alt="QR Code de Autenticação"
              style={{ width: `${config.versoQrTamanho || 90}px`, height: `${config.versoQrTamanho || 90}px` }}
          ...
          <span className="text-[6.5px] font-mono font-bold text-neutral-800 tracking-tighter mt-0.5 uppercase">
            VERIFICAÇÃO OFICIAL
          </span>
        </div>

        {/* Individual Authentication Details */}
        <div className="flex-1 min-w-0 space-y-1.5 pl-1">
          <div>
            ...
            <span
              ...
              style={{
                ...
                fontSize: `${config.versoCodigoTamanho || 12}px`,
              }}
            >
              {credential.authCode}
            </span>
          ...
          <div className="grid grid-cols-2 gap-2 pt-0.5">
            {config.versoExibirRegistro !== false && (
              <div>
                <span className="text-[6.5px] text-neutral-400 font-bold uppercase tracking-wider block">
                  REGISTRO
                </span>
                <span className="text-white font-mono text-[9.5px] font-bold block truncate">
                  {credential.registro}
                </span>
              </div>
            )}
            ...
            {config.versoExibirValidade !== false && (
              <div>
                ...
                <span className="text-white font-black text-[9.5px] block truncate">
                  {validadeStr}
                </span>
              </div>
            )}
          ...
          <div className="pt-0.5">
            <span className="text-[6.5px] text-neutral-500 font-mono font-bold block truncate">
      ...
      {config.versoExibirMensagem !== false && (
        <div className="z-10 border-t border-neutral-800/80 pt-1.5 flex items-center gap-1.5">
          <Lock className="w-3 h-3 text-orange-500 shrink-0" />
          <p className="text-[7px] text-neutral-400 font-medium leading-tight">
            {config.versoMensagemSeguranca ||
              'Carteirinha oficial. Autentique esta identificação através do QR Code ou código de autenticação no sistema Arena do Competidor.'}
          </p>
        </div>
      )}
    </div>
